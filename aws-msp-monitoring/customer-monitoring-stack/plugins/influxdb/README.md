...
This plugin provides influxdb integration for the monitoring stack.
...
Edit `influxdb-config.yml` for advanced configuration options.
...
scrape_configs:
  - job_name: 'influxdb'
    static_configs:
      - targets: ['localhost:9208']
    scrape_interval: 60s
```

### Grafana Dashboard
Import dashboard for influxdb metrics visualization.
...
## Support
- Configuration: Edit `influxdb-config.yml`
