...
The **Operations Observability Stack** is a reference monitoring and observability baseline: Prometheus, Grafana, secure metrics API, Pushgateway, and 49 plugin templates. Use it to learn architecture patterns, run a local lab, or hand off a hardened starting point to a customer environment.

**Note:** This is a starting point, not an AWS fully managed or supported production solution. Validate your own implementation against your organization's security and compliance requirements.
...
#### **macOS/Linux Users:**
1. **Open Terminal**
2. **Navigate** to the extracted folder: `cd path/to/observability-stack`
3. **Run:** `./install.sh`
4. **Done!** Grafana opens automatically

### **Step 3: Access Your Monitoring**
- **Grafana Dashboard:** http://localhost:3000
- **Login:** `admin` / `GRAFANA_ADMIN_PASSWORD` in `customer-monitoring-stack/.env`
- **Default dashboard:** Operations in a Box - Overview (provisioned on install)

### **Credentials and install layout**

The installer writes secrets to `customer-monitoring-stack/.env` (not into git-tracked config). Docker Compose injects those values into Grafana and the metrics API at runtime. See [Installation & Credentials](../README.md#installation--credentials) in the root README for the full variable list and API usage examples.
...
- Enterprise-style monitoring stack (Prometheus, Grafana, API, Pushgateway)
- Security patterns and hardening examples (authentication and encryption)
- **49 plugins across 12 categories** (complete ecosystem coverage)
- **Complete dashboard tutorial** (step-by-step beginner to advanced)
- **API integration examples** (Python, JavaScript, curl)
- **Demo data generator** (realistic metrics for testing)
- **Comprehensive documentation** (6 detailed guides)
...
### **For Beginners:**
1. **Read:** `DASHBOARD_WALKTHROUGH.md` - Complete tutorial
...
2. **Security analysis:** `SECURITY_ANALYSIS.md`
...
- **Advanced Security** - SAML/OAuth, Certificates, Network segmentation (3 plugins)
- **Cloud Integration** - AWS CloudWatch, AWS CloudTrail, AWS Config, AWS VPC Flow Logs, AWS Reachability Analyzer, Auto-Discovery, Cost optimization, MontyCloud (8 plugins)
...
- **Ticketing Platform Integration** - AWS Support, Zendesk, Freshworks, Linear (4 plugins)
...
- `demo_data_generator.py` - Demo data generator (fixed metric names)
...
2. **Security issues:** `SECURITY_GUIDE.md`
...
### Scaling Guidelines
- **Small deployment (1-15 plugins)**: 4 CPU, 8GB RAM, 20GB disk
- **Medium deployment (15-30 plugins)**: 8 CPU, 16GB RAM, 50GB disk  
- **Enterprise deployment (30+ plugins)**: 16+ CPU, 32GB+ RAM, 100GB+ disk
...
**Important:** This project is not an official AWS product. You are responsible for validating deployments against your organization's requirements.
