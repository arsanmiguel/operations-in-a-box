...
def load_api_key(env_path="customer-monitoring-stack/.env"):
    """Read API_KEY from the local .env file."""
    paths = [Path(env_path), Path("customer-monitoring-stack/.env"), Path(".env")]
    for path in paths:
        if not path.exists():
            continue
