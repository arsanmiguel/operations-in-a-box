...
3. **Login with:**
   - Username: `admin`
   - Password: (from `GRAFANA_ADMIN_PASSWORD` in `customer-monitoring-stack/.env`)
...
- **Method:** `POST`
- **Authentication:** API Key (from `API_KEY` in `.env`)
- **Format:** JSON
- **Rate Limits:** 100 requests/hour, 10 requests/minute

#### 9.2 Get Your API Key
1. **Open** `customer-monitoring-stack/.env`
...
```bash
# Replace YOUR_API_KEY with the API_KEY value from .env
