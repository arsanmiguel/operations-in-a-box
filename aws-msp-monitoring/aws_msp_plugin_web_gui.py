...
def require_gui_auth(f):
    """Require API key for plugin install/uninstall/restart operations.
    
    Set GUI_API_KEY env var to enable. If unset, falls back to
    localhost-only binding (see __main__ block) as the security boundary.
    """
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if not GUI_API_KEY:
            # No key configured; rely on localhost binding as security boundary
            return f(*args, **kwargs)
        provided_key = request.headers.get('X-GUI-Key') or request.args.get('key')
        if not provided_key or not hmac.compare_digest(provided_key, GUI_API_KEY):
            return jsonify({'error': 'Unauthorized. Set X-GUI-Key header or ?key= param.'}), 401
        return f(*args, **kwargs)
