...
### Secure API Usage
```bash
# Get API key from customer-monitoring-stack/.env
...
- Secure credential generation into local `.env` (not committed to git)
- Docker Compose loads `GRAFANA_ADMIN_PASSWORD`, `GRAFANA_SECRET_KEY`, and `API_KEY` from `.env` at runtime
...
### Credential handoff to customers

Each deployment gets a unique `customer-monitoring-stack/.env` created during install. Transfer this file through your normal secure channel (password manager, encrypted share, etc.). Do not email it in plain text or check it into version control. Grafana login uses `admin` and `GRAFANA_ADMIN_PASSWORD`; integrations use `API_KEY`.
...
4. **Authentication failures** - Verify `API_KEY` in `customer-monitoring-stack/.env`
