...
This plugin provides mongodb integration for the monitoring stack.
...
curl http://localhost:9152/health

# View metrics
curl http://localhost:9152/metrics
...
Edit `mongodb-config.yml` for advanced configuration options.
...
scrape_configs:
  - job_name: 'mongodb'
    static_configs:
      - targets: ['localhost:9152']
    scrape_interval: 60s
```

### Grafana Dashboard
Import dashboard for mongodb metrics visualization.
...
## Support
- Configuration: Edit `mongodb-config.yml`
