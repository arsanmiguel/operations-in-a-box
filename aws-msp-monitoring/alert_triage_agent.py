...
# Maximum allowed size for alert payloads (prevent abuse)
MAX_ALERT_PAYLOAD_BYTES = 10000

def sanitize_alert_data(alert_data):
    """Sanitize alert payload before embedding in Bedrock prompt.
    
    Prevents prompt injection by stripping control sequences and
    truncating excessively large payloads.
    """
    raw = json.dumps(alert_data, indent=2, default=str)
    # Truncate oversized payloads
    if len(raw) > MAX_ALERT_PAYLOAD_BYTES:
        raw = raw[:MAX_ALERT_PAYLOAD_BYTES] + "\n[TRUNCATED]"
    # Strip sequences that could be interpreted as prompt boundaries
    raw = re.sub(r'(Human:|Assistant:|<\|.*?\|>|<<SYS>>|<</SYS>>)', '[REDACTED]', raw)
    return raw


class AlertTriageAgent:
    def __init__(self, bedrock_model="anthropic.claude-3-5-sonnet-20241022-v2:0", region=None):
        region = region or os.environ.get('AWS_REGION', 'us-east-1')
        self.bedrock = boto3.client('bedrock-runtime', region_name=region)
        self.model = bedrock_model
        # Validate IAM permissions on init
        try:
            self.bedrock.meta.service_model
        except Exception as e:
            raise RuntimeError(
                f"Failed to initialize Bedrock client in {region}. "
                f"Verify IAM permissions (bedrock:InvokeModel). Error: {e}"
            )
        
    def analyze_alert(self, alert_data):
        """Use Bedrock to analyze alert and determine routing"""
        
        # Sanitize input to prevent prompt injection from alert payloads
        safe_alert_json = sanitize_alert_data(alert_data)
