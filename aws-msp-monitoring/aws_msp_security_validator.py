    ...
    def check_secure_credentials(self):
        ...
        if credentials_file.exists():
            ...
            if "admin/admin" not in content and len(content) > 100:
                self.passed_checks.append("✅ Secure credentials generated (no default admin/admin)")
                return True
        
        self.failed_checks.append("❌ Default credentials still in use")
        return False
    ...
    def check_api_authentication(self):
        ...
        if api_file.exists():
            ...
            if "require_api_key" in content and "hmac.compare_digest" in content:
                self.passed_checks.append("✅ API authentication implemented with secure key comparison")
                return True
                
        self.failed_checks.append("❌ API endpoints not authenticated")
        return False
    ...
    def check_container_security(self):
        ...
        if compose_file.exists():
            ...
            if 'user: "65534:65534"' in content and 'user: "472:472"' in content and 'user: "1000:1000"' in content:
                self.passed_checks.append("✅ All containers configured to run as non-root users")
                return True
                
        self.failed_checks.append("❌ Containers running as root")
        return False
    ...
    def check_network_security(self):
        ...
        if compose_file.exists():
            ...
            if "127.0.0.1:9090:9090" in content and "127.0.0.1:3000:3000" in content and "127.0.0.1:8080:8080" in content:
                self.passed_checks.append("✅ All services bound to localhost only (127.0.0.1)")
                return True
                
        self.failed_checks.append("❌ Services exposed on all interfaces")
        return False
    ...
    def check_security_headers(self):
        ...
        if api_file.exists():
            ...
            if all(header in content for header in headers):
                self.passed_checks.append("✅ Security headers implemented (XSS, CSRF, clickjacking protection)")
                return True
                
        self.failed_checks.append("❌ Security headers not configured")
        return False
    ...
    def check_input_validation(self):
        ...
        if api_file.exists():
            ...
            if "validate_metric_name" in content and "re.match" in content:
                self.passed_checks.append("✅ Input validation implemented with regex patterns")
                return True
                
        self.failed_checks.append("❌ Input validation not implemented")
        return False
    ...
    def check_rate_limiting(self):
        ...
        if api_file.exists():
            ...
            if "flask_limiter" in content and "limit(" in content:
                self.passed_checks.append("✅ Rate limiting configured (100/hour, 10/minute)")
                return True
                
        self.failed_checks.append("❌ Rate limiting not configured")
        return False
    ...
    def check_session_security(self):
        ...
        if compose_file.exists():
            ...
            if "GF_SESSION_LIFE_TIME=3600" in content:
                self.passed_checks.append("✅ Session timeout configured (1 hour)")
                return True
                
        self.failed_checks.append("❌ Session timeout not configured")
        return False
    ...
    def check_container_hardening(self):
        ...
        if compose_file.exists():
            ...
            if all(feature in content for feature in security_features):
                self.passed_checks.append("✅ Container hardening enabled (read-only, no privileges, dropped capabilities)")
                return True
                
        self.failed_checks.append("❌ Container hardening not implemented")
        return False
